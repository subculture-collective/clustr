...
metrics="${rehearsal_root}/publication-time.txt"
log_file="${rehearsal_root}/publication.log"
...
if [[ -e "${metrics}" || -e "${log_file}" ]]; then
  echo "publication evidence already exists; refusing to overwrite it" >&2
  exit 1
fi
...
started_ns=$(date +%s%N)
"${binary}" --once --publish-only >"${log_file}" 2>&1 &
worker_pid=$!
cleanup() {
...
if ! wait "${worker_pid}"; then
  echo "publication failed; inspect ${log_file}" >&2
  exit 1
fi
