...
binary="${rehearsal_root}/clustr-precalculate"
metrics="${rehearsal_root}/calculation-time.txt"
log_file="${rehearsal_root}/calculation.log"
...
if [[ -e "${metrics}" || -e "${log_file}" ]]; then
  echo "calculation evidence already exists; refusing to overwrite it" >&2
  exit 1
fi
...
export DETAILED_GRAPH=${DETAILED_GRAPH:-false}
export LAYOUT_MAX_NODES=${LAYOUT_MAX_NODES:-5000}
export GRAPH_COMMUNITY_MAX_NODES=${GRAPH_COMMUNITY_MAX_NODES:-10000}
export GRAPH_COMMUNITY_MAX_LINKS=${GRAPH_COMMUNITY_MAX_LINKS:-50000}
...
export GRAPH_REVISION_RETENTION=${GRAPH_REVISION_RETENTION:-24}
export GRAPH_PUBLICATION_WORK_MEM_MB=${GRAPH_PUBLICATION_WORK_MEM_MB:-512}

started_ns=$(date +%s%N)
"${binary}" --once --full >"${log_file}" 2>&1 &
worker_pid=$!
cleanup() {
...
if ! wait "${worker_pid}"; then
  echo "calculation failed; inspect ${log_file}" >&2
  exit 1
fi
